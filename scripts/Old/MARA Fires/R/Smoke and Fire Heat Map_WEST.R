...
# Make sure `smoke_west` is projected correctly
smoke_west_projected <- st_transform(combined_shapefiles_west, crs = 32612)

# Count the number of overlaps for each grid cell
grid_sf$overlap_count <- lengths(st_intersects(grid_sf, smoke_west_projected))
...
common_cols <- intersect(intersect(names(CAN_fires_west), names(US_fires_west)), names(US_fires_west_2023))

# Transform coordinates of each dataset to EPSG:32612
CAN_fires_west <- st_transform(CAN_fires_west, crs = 32612)
US_fires_west <- st_transform(US_fires_west, crs = 32612)
US_fires_west_2023 <- st_transform(US_fires_west_2023, crs = 32612)

# Subset and rename columns if necessary
CAN_fires_west <- CAN_fires_west[, common_cols]
US_fires_west <- US_fires_west[, common_cols]
US_fires_west_2023 <- US_fires_west_2023[, common_cols]

# Combine all datasets into one
combined_fires_west <- rbind(CAN_fires_west, US_fires_west, US_fires_west_2023)

# Check for invalid geometries
invalid_geom <- st_is_valid(combined_fires_west)
...
combined_fires_west <- st_make_valid(combined_fires_west)

# Simplify geometries
combined_fires_west_simplified <- st_simplify(combined_fires_west, dTolerance = 1000)  # Adjust tolerance as needed
...
combined_fires_west_simplified <- st_crop(combined_fires_west_simplified, raster_bbox)
...
ggplot() + 
  ...
  geom_sf(data = combined_fires_west_simplified, fill = "black", color = "black", size = 0.5, alpha = 1) + 
  ...
  labs(title = bquote(atop(
    bold("WESTERN MT 2003-2023 Highest 1% PM"[2.5]*" Days"), 
