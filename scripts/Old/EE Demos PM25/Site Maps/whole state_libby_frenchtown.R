...
# Filter to get the latitude and longitude for "frenchtown"
frenchtown_coords <- sites_simple %>%
  filter(site == site_name) %>%
  select(latitude, longitude) %>%
  slice(1)  # Ensures only one row is selected if there are duplicates
...
# Calculate the bounding box based on the buffer distance
bbox_frenchtown <- c(
  left = frenchtown_coords$longitude - buffer,
  bottom = frenchtown_coords$latitude - buffer,
  right = frenchtown_coords$longitude + buffer,
  top = frenchtown_coords$latitude + buffer
)

# Create a polygon from the bounding box coordinates
bbox_frenchtown_polygon <- st_polygon(list(rbind(
  c(bbox_frenchtown["left"], bbox_frenchtown["bottom"]),  # Bottom-left corner
  c(bbox_frenchtown["right"], bbox_frenchtown["bottom"]), # Bottom-right corner
  c(bbox_frenchtown["right"], bbox_frenchtown["top"]),    # Top-right corner
  c(bbox_frenchtown["left"], bbox_frenchtown["top"]),     # Top-left corner
  c(bbox_frenchtown["left"], bbox_frenchtown["bottom"])   # Closing the polygon
))) %>% 
  st_sfc(crs = 4326)  # Set coordinate reference system to WGS84 (lat/lon)

# Convert to a data frame for plotting in ggplot
bbox_frenchtown_polygon_df <- st_as_sf(bbox_frenchtown_polygon)
...
ggplot() + 
  ...
  geom_sf(data = bbox_frenchtown_polygon_df, fill = "white", color = "white", size = 1.5, alpha = 0.4) +
