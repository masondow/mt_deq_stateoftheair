...
start_date <- Sys.Date() - years(2) # this operational version of the merge script only looks at last 2 years of data and then anti-joins the changes to the append-only file
# to initialize the entire available data archive, see data_scripts/processing/"pollutant"/utilities

AirNow_parameter_name <- "PM2.5"

AirNow_hourly <- readRDS(here("data", "AirNow", "hourly_AirNow_all_MT.rds")) %>%
  filter(date_local >= start_date)
AirNow_daily <- readRDS(here("data", "AirNow", "daily_AirNow_all_MT.rds")) %>%
  filter(date_local >= start_date)
AQS_hourly_PM25 <- readRDS(here("data","PM25","hourly_PM25_all_MT.rds")) %>%
  filter(date_local >= start_date)
AQS_daily_PM25 <- readRDS(here("data","PM25","daily_PM25_all_MT.rds")) %>%
  filter(date_local >= start_date)
wf_flags_PM25 <- readRDS(here("data","PM25","wf_flags_PM25_all_MT.rds"))

site_names <- readRDS(here("data","sites.rds")) %>%
  select(site_name, local_site_name, county_code, site_number) %>%
  distinct()
AQS_pocs_PM25 <- readRDS(here("data", "PM25","AQS_PM25_pocs.rds"))


# Filter the AQS data to only include AQS primary pocs
AQS_hourly_PM25 <- AQS_hourly_PM25 %>%
  semi_join(AQS_pocs_PM25, by = c("county_code", "site_number", "poc"))
AQS_daily_PM25 <- AQS_daily_PM25 %>%
  semi_join(AQS_pocs_PM25, by = c("county_code", "site_number", "poc"))

#### Last Best POC ####
# use to apply to AirNow data
poc_values <- AQS_pocs_PM25 %>%
...
# AQS cleaning
AQS_hourly <- as.data.table(AQS_hourly_PM25) # Convert to data.table for faster processing
...
AQS_hourly_dt <- AQS_hourly_dt[, c(common_columns, "parameter_code", "sample_duration", "qualifier"), with = FALSE]
...
# Merge updates with full record
merged_full_record <- readRDS(here("data_processed","merged_hourly", "merged_hourly_PM25.rds"))

key_cols <- c("county_code", "site_number", "poc", "date_time")

# Optional: Ensure date_time is POSIXct if needed
# merged_2yr$date_time <- as.POSIXct(merged_2yr$date_time)
# merged_full_record$date_time <- as.POSIXct(merged_full_record$date_time)

updates <- merged_2yr %>%
  anti_join(merged_full_record, by = key_cols)

merged_full_record_updated <- bind_rows(merged_full_record, updates)

merged_full_record_updated <- merged_full_record_updated %>% # Optionally arrange and remove duplicates (in case of weird overlaps)
  distinct(across(all_of(key_cols)), .keep_all = TRUE)

merged_full_record_short <- merged_full_record_updated %>% # Save a recent-only
  filter(date_time >= Sys.time() - lubridate::days(4))

saveRDS(merged_full_record_updated, here("data_processed","merged_hourly", "merged_hourly_PM25.rds"))
saveRDS(merged_full_record_short, here("data_processed","merged_hourly", "recent_hourly", "merged_short_hourly_PM25.rds"))


#### DAILY MERGE ####

# AQS cleaning
AQS_daily <- AQS_daily_PM25 %>%
  filter(validity_indicator == "Y") %>%
  left_join(wf_flags_PM25 %>%
...
# Merge updates with full record
merged_full_record <- readRDS(here("data_processed","merged_daily", "merged_daily_PM25.rds")) 
...
saveRDS(merged_full_record_updated, here("data_processed","merged_daily", "merged_daily_PM25.rds"))
